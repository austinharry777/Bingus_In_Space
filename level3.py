...
from level3boss import Level3Boss
from pause import Pause
from resources import *
pygame.init()

music = pygame.mixer.Sound('sounds/bingus level3.wav')
music.set_volume(0.9)

class Level3(Base_State):
    def __init__(self, game, score, health):
        ...
        self.damage_ship_group = pygame.sprite.Group()
    ...
    def laser_collision(self):
        if self.dead_floppas >= 30:
            for floppa in self.meteor_group:
                self.explosion = Explosion(floppa.rect.center, (floppa.meteor_size.x * 1.2))
                self.explosion_group.add(self.explosion)
            music.fadeout(2000)
            self.meteor_spawn_time = 300000
            level3boss = Level3Boss(self.game, self.score, self.health, self.explosion_group, self.ship, self.background)
            level3boss.enter_state()
    ...
    def floppa_counter(self, display_surface):
        self.floppa_surf = self.font.render(f"FLOPPAS REMAINING: {30 - self.dead_floppas}", True, (255,255,255))
...
class Meteor(pygame.sprite.Sprite):
    def __init__(self, pos):
        super().__init__()
        #randomize floppa size
        meteor_surf = pygame.image.load('graphics/floppajetpack.png').convert_alpha()
        self.meteor_size = pygame.math.Vector2(meteor_surf.get_size()) * uniform(1.5, 2.5)
...
class Background3:
    def __init__(self):  
        self.background_surf = pygame.image.load('graphics/space.png').convert()
        self.background_surf_height = self.background_surf.get_height()
        self.background_surf2 = pygame.image.load('graphics/asteroids.png').convert_alpha()
