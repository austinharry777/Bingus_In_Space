...
class Cutscene1(Base_State):
    def __init__(self, game, score, health):
        ...
        self.game = game
        self.load_assets() 
        self.bing_orig = "BINGUS FLY HIGH."
        self.bing_orig1 = "NOTHING STOP BINGUS NOW."
        self.bing_orig2 = "BINGUS REMEMBER FLOPPA WAS FRIEND."
        self.bing_orig3 = "WHAT HAPPEN FLOPPA?"
        self.flop_orig = "BINGUS NO FLY."
        self.flop_orig1 = "FLOPPA MAD AT BINGUS."
        self.flop_orig2 = "FLOPPA STOP BINGUS."
        self.flop_orig3 = "FLOPPA FLY TO MOON INSTEAD."
    ...
    def update(self, dt, actions):
        ...
        if self.done9 == True:
            ...
            if self.fade.fade_complete == True:
                level2 = Level2(self.game, self.score, self.health)
                level2.enter_state()
    ...
    def load_assets(self):
        self.bingusimage = pygame.image.load('graphics/bingus intro pic.png').convert()
        self.floppaimage = pygame.image.load('graphics/cutscenefloppa.png').convert()
    ...
    def draw(self, display_surface):
        ...
        if pygame.time.get_ticks() - self.current_time > self.delay:
            ...
            if pygame.time.get_ticks() - self.current_time > self.delay * 2:
                self.draw_text(display_surface, self.flop_orig[:int(self.k)], 430, 420, c.WHITE)
                    ...
                    if pygame.time.get_ticks() - self.current_time > self.delay * 4:
                        ...
                        if pygame.time.get_ticks() - self.current_time > self.delay * 5:
                            ...
                            if pygame.time.get_ticks() - self.current_time > self.delay * 6:
                                pygame.draw.rect(display_surface, c.BLACK, (-90, 400, 1000, 1000))
                                ...
                                if pygame.time.get_ticks() - self.current_time > self.delay * 7:
                                    self.draw_text(display_surface, self.flop_orig3[:int(self.p)], 300, 420 + 75, c.WHITE)
                                    if pygame.time.get_ticks() - self.current_time > self.delay * 7 + self.delaytext2:
