...
from level_complete2 import LevelComplete2
from pause import Pause
from resources import *
pygame.init()
pygame.mixer.init()

boss_music = pygame.mixer.Sound('sounds/boss2.wav')
boss_music.set_volume(0.8)


class Level2Boss(Base_State):
    ...
    def update(self, dt, actions):
        ...
        self.boss.bomb_group.update(dt)
        ...
        if self.boss_dead == True:
            ...
            if self.boss_explosion.animation_done == True:
                ...
                if pygame.time.get_ticks() - self.boss_explosion.boss_death_timer > 3000:
                    self.game.completed_levels += 1 #counter for health bonus
                    lc2 = LevelComplete2(self.game, self.score, self.health)
                    lc2.enter_state()
    ...
    def draw(self, display_surface):
        ...
        self.boss.bomb_group.draw(display_surface)
    ...
    def bomb_collision(self):
        for bomb in self.boss.bomb_group:
            if pygame.sprite.collide_mask(self.ship, bomb) and self.vulnerable == True:
                self.ship.explosion_sound.play()
                self.explosion = Explosion(self.ship.rect.center, (self.ship.rect.width * 1.2))
                self.explosion_group.add(self.explosion)
                self.ship.damage_ship = True
